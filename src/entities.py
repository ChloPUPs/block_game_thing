...
class Player:
    ...
    def update(self, camera_x, camera_y, level, key_down_dict):
        ...
        for b in level["level_data"]["blocks"]:
            while self.x < b["pos"]["x"] + 1*0.95 and self.x + 4/5*0.9 > b["pos"]["x"] and self.y < b["pos"]["y"] + 1 / 100 and self.y + 4/5 > b["pos"]["y"]:
                self.y -= 0.01
                self.velocity_y = 0
            while self.x < b["pos"]["x"] + 1*0.95 and self.x + 4/5*0.9 > b["pos"]["x"] and self.y < b["pos"]["y"] + 1 and self.y + 4/5 / 100 > b["pos"]["y"]:
                self.y += 0.01
                self.velocity_y = 0
